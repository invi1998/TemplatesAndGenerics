...
这种就说明是A类包含B对象，实心菱形在哪边，哪边就是包含，箭头指向哪个，哪个就是被包含
...
递归继承展开参数包：每个实例化出来的类this指针都是相同的【实例化出来的类是继承关系】

![](../img/2_15_2.png)

递归组合展开参数包：每个实例化出来的类this指针都不相同【实例化出来的类是包含（组合方式）】

![](../img/2_15_3.png)

编译器在实例化myclasst<int, float, double>的时候，发现他里面有个myclass<float,double>类型的成员变量，

那么编译器就会先去实例化myclass<float,double>，然后在实例化myclass<float,double>的时候，发现他里面有一个

myclass<double>类型的成员变量，那么编译器就会先去实例化这个myclass<double>，然后实例化myclass<double>

的时候，发现他里面有个myclass<>，编译器就会先去实例化myclass<>。
